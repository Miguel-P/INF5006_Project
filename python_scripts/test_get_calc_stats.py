...
# need to use patch and mock to check if it is reading in data from external
# sources properly properly

class TestGet(unittest.TestCase):

    # this is for a setup and teardown functions that only run at the beginning
    # at end of
    # all the tests
    @classmethod
    def setUpClass(cls):
        ...
        cls.indx_tbl = pd.read_csv(
            "python_scripts//Tables_CSV//tbl_Index_Constituents.csv",
            parse_dates=["Date"])
        cls.beta_tbl = pd.read_csv(
            'python_scripts//Tables_CSV//tbl_BA_beta_Output.csv',
            parse_dates=["Date"])
    ...
    def test_GetICsandWeights(self):
        ...
        self.assertTrue(math.isclose(1.0, sum(weights),
                                     rel_tol=1e-6, abs_tol=0.0))

    def test_GetBetasMktAndSpecVols(self):
        IC, weights = GetICsAndWeights(self.indx_tbl, self.x, "ALSI")

        # testing on using IC, weights (determined from previous function)
        # and J200 index
        betas, specVols, mktVol = GetBetasMktAndSpecVols(self.beta_tbl,
                                                         self.x, IC, "J200")
    ...
    def test_CalcStats(self):
        ...
        (pfBeta, sysCov, pfSysVol, specCov, pfSpecVol, totCov, pfVol, CorrMat,
         indx_nan) = CalcStats(weights, betas, mktVol, specVols)
        # check if all the shape of the outputs are correct
        (self.assertTrue(pfBeta.shape == (1, 1) and
         pfSysVol.shape == (1, 1) and
         pfSpecVol.shape == (1, 1) and pfVol.shape == (1, 1)))
        (self.assertTrue(sysCov.shape[0] == sysCov.shape[1] and
         sysCov.shape == totCov.shape and totCov.shape == CorrMat.shape and
         specCov.shape == CorrMat.shape))

        # check if nan indx is correct (known from manually checking the point)
        self.assertEqual(indx_nan, [104])

        # make sure diagonals of correlation matrix are approximately 1
        # use list comprehension to make list containing False if element
        # is 1 and True if not
        Not_1 = [not math.isclose(1.0, i, rel_tol=1e-6, abs_tol=0.0)
                 for i in np.diag(CorrMat)]
