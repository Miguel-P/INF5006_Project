...
# function takes in panda df tbl, datetime rDate and string indexCode which is one of
# "ALSI", "FLED", "LRGC", "MIDC", "SMLC", "TOPI", "RESI", "FINI", "INDI", "PCAP", "SAPY" or "ALTI"
# and outputs 2 numpy arrays IC containg the share codes and the corresponding market cap. weights (np.array of type float)
def GetICsAndWeights(tbl,rDate,indexCode):
    # define output arrays
    IC = []
    weights = []
    # define dictionary to map indexCode to respective column name
    indxCode2col = {"ALSI":"ALSI New","FLED":"ALSI New","LRGC":"Index New", "MIDC":"Index New", "SMLC":"Index New", "TOPI":"TOPI New", "RESI":"RESI New", "FINI":"FINI New", "INDI":"INDI New", "PCAP":"PCAP New", "SAPY":"SAPY New", "ALTI":"ALTI New"}
    temp_tbl = tbl[(tbl["Date"].dt.year == rDate.year) & (tbl["Date"].dt.month == rDate.month) & (tbl[indxCode2col[indexCode]] == indexCode)]
    
    # get outputs as numpy arrays
    IC = np.array(temp_tbl["Alpha"])
    weights = np.array(temp_tbl["Gross Market Capitalisation"])
    
    # Gross Market Capitilisation using commas instead of . for decimal point use list comprehension to rectify
    # pandas reads Nulls as nan floats so essentially checking if it is a float and a nan 
    weights = [i if isinstance(i,float) else float(i.replace(",",".")) for i in weights]
...
def GetBetasMktAndSpecVols(tbl,rDate,IC,mktIndexCode):
    temp_tbl = tbl[(tbl["Date"].dt.year == rDate.year) & (tbl["Date"].dt.month == rDate.month) & (tbl["Index"] == mktIndexCode)]
    # get rows where Instrument is contraining in IC array
    temp_tbl = temp_tbl[temp_tbl["Instrument"].isin(IC)]
    
    betas = temp_tbl["Beta"]
#     betas = np.array(temp_tbl["Beta"])
    
    # list comprehension to remove commas with handling for nans
    betas = [i if isinstance(i,float) else float(i.replace(",",".")) for i in betas]
    betas = np.array(betas)
    
    specVols = np.array(temp_tbl["Unique Risk"])
    
    specVols = [i if isinstance(i,float) else float(i.replace(",",".")) for i in specVols]
    specVols = np.array(specVols)
    
    # get mktVol string val
    mktVol = tbl[(tbl["Date"].dt.year == rDate.year) & (tbl["Date"].dt.month == rDate.month) & (tbl["Instrument"] == mktIndexCode) & (tbl["Index"] == mktIndexCode)]["Total Risk"]
